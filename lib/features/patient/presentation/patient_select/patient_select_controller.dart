...
class PatientSelectController extends StateNotifier<PatientSelectState> {
  PatientSelectController(this.patientService)
      : super(
          PatientSelectState(),
        );
  ...
  Future<void> _loadData(int pageKey) async {
    final result = await patientService.selectPatientUsers(
      PaginationRequest(
        page: pageKey,
...
final patientSelectControllerProvider = StateNotifierProvider.autoDispose<
    PatientSelectController, PatientSelectState>(
  (ref) {
    return PatientSelectController(ref.read(patientServiceProvider));
  },
);
